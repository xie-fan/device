...
**跑几台由过滤粒度决定**，没有 `--random` / `--all` 这类开关：

| 给的条件 | 跑几台 |
|---|---|
| `device_id` | 就那一台 |
| 只给到 `--device-type` | 该类型下**随机一台** |
| 只给 `--env` / `--enterprise` | 命中的**全部**（默认串行，`--parallel` 才并行） |
| 什么都不给 | 全部 |

用户说「测一下 X 机型」时给 `--device-type` 就够，不必先 `devices` 挑 id。**要跑遍某个厂商或环境下的全部设备，是批量任务**：先用 `devices` 配合相同过滤核对范围，只运行用户要求的目标。
