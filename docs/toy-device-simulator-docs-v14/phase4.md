...
- queue：长度、入队超时、取消表（与 writePump 有界队列 / BeginClose 不是同一功能）
- 线上 mp3/wav 推流：整段 PCM 只编码一次再切流
- HTTP 上传 raw PCM（须带完整 sample_rate/channels/sample_format；当前 Phase 2 明确不做）
- 全局跨设备事件总线（当前 event_seq 仅为设备级）
