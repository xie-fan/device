...
**CancelTurn：** 不置 `closing`；只删本 Turn 未写出的 Stage 1/2；按取消表追加 Stage 3（占 outbound 控制槽）；ACK/report 保留。已从队列取出正在 Write 的帧视为已发出。

**BeginClose：** 仅进程收口 / 连接失败。`closing=true`；清空 Stage=1/2 与 ACK/report 等数据帧；**保留已入队 Stage=3**。优雅关闭且 token=Stage3 时，若尚无该 uuid 的 Stage=3 再追加为最后一帧。异常 / Reserved / 已 Terminal：不追加新 Stage=3，但 CancelTurn 已入队的 Stage=3 必须由 Phase B 写出后再关 socket。closer 协程跑 drain/close。

失败 JSON：读循环不得关 socket；持锁 CancelTurn + `terminalLocked(error)`，解锁后唤醒。连接保持到 CLI 退出再 BeginClose（保留已入队 Stage=3）。
...
|------|------------------------|----------------------|----------------------|
| Reserved | 不发 Stage=3 | 不发 Stage=3；清空 buffer | 保持空 |
...
ACK∥timeout 一次消费；失败 JSON 发 Stage=3 且 **连接未因 CancelTurn 关闭**；进程退出 BeginClose 丢未发 Stage 1/2；最后一帧 Stage=3 来自本次 token **或** 先前 CancelTurn 已入队者（已 Terminal 的 token=无不得清空它）；`terminalLocked` 不在锁内唤醒；解码失败不占槽；全部终态有 `turn_terminal` 且字段符合 §6 终止表。
