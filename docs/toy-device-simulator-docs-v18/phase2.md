# Phase 2 详细设计：多设备编排 + API + Scenario（v18）
...
`POST /devices/{id}/stop`、`DELETE /devices/{id}` 按 §4。DELETE 200 `{ "device_id","instance_id","deleted": true }`。stop/delete 走 BeginClose，不走 CancelTurn。若此前 `/interrupt` 或失败 JSON 已 `CancelTurn` 入队 Stage=3，BeginClose(token=无) **必须保留并写出** 该帧，不得当作已 Terminal 而清空 buffer。
...
speak_and_wait 200 另含终态：`turn_end_reason`、`uplink_end_reason`、`reply_kind`、`event_seq`、`event_type`（通常 `turn_terminal`）。取值见 §6.7 终止表。504 时 Turn 继续。completion waiter 由 `terminalLocked` 摘出；同一事件的 `/wait` event waiter 由其中 `appendEventLocked` 摘出；外层解锁后两者都唤醒，并 WS fan-out。
...
- [ ] `terminalLocked` 不重复加锁；经 `appendEventLocked` 摘 event waiter 并快照 WS；Phase C 只摘一次；解锁后才唤醒 completion **和** event waiter，并 WS fan-out。禁止日志已有 `turn_terminal` 而 `/wait` 仍 504
...
- [ ] BeginClose 后未发 Stage=2 不会发出；Stage=3 为连接最后一帧。CancelTurn 已入队的 Stage=3 在随后 BeginClose(token=无) 时仍写出
- [ ] outbound 为 Stage=3 留 1 个控制槽；数据帧满不阻止 Stage=3；Stage=3 入不了队则 `local_validation_error`/`stage3_backpressure`，`/interrupt` 仍 200
