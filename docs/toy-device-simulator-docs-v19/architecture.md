...
DeviceInstance:
  ...
  BeginClose：closing=true；**过滤**保留已入队 Stage=3，丢掉其它未写出帧；可选再追加最后一帧；然后关连接
  request_finalize 三段 + join-wait
  terminalLocked：锁内迁移 Turn；经 appendEventLocked 写事件；累积 EventNotify；外层解锁后唤醒 completion+event waiter 并 WS fan-out
...
任何事件写入（`turn_terminal`、`tts_done`、`command_received`、连接生命周期、`local_validation_error` 等）**必须**走本函数。禁止「只 append 日志、等 /wait 自己再看见」。

返回值 `EventNotify`：`{eventWaiters, event, wsChans}`。

- 调用方 **必须已经持有** `device_mu`。不得再加锁、不得 IO、不得唤醒、不得 `WriteMessage`。
- 步骤：分配 `event_seq` 并 append 日志 → 摘走匹配的 live **event waiter** → 快照本 instance 的 live WS 发送通道（只拷贝 chan 引用）→ 返回 `EventNotify`。
...
**同一临界区累积：** 一次持锁内可能连续写入 `tts_done` 再 `turn_terminal`（或 `protocol_error` 再 `turn_terminal`）。每次 `appendEventLocked` 的 `EventNotify` **必须累加**到调用方的 `acc`。丢弃中间返回值会使 waiter 已从表中摘走却永不 200（也不会 504）。`terminalLocked` 内部那一次也并入 `acc`。
...
for e in acc:
  notify(e.eventWaiters)       # POST /wait；禁止留在表里空等到 504
  wsFanout(e.event, e.wsChans) # live WS 推未来；禁止在 device_mu 内 WriteMessage
```
...
2. **先**从 outbound buffer 删除该 `turn_id` 尚未写出的 Stage=1 与 Stage=2（给控制槽腾位）。不得删除 ACK、report、其它 turn_id、已入队的 Stage=3。
...
for frame in outbound buffer:
  if frame 是尚未写出的 Stage=3: keep.append(frame)   # 每 uplink_uuid 至多保留一帧
  else: 丢弃（Stage=1/2、ACK、report、keepalive）
正在 Write 的帧视为已发出，不进入 keep、也不撤回
if token == Stage3:
  若 keep 与正在 Write 中都没有该 Turn uplink_uuid 的 Stage=3:
    keep.append(新 Stage=3)   # 本连接最后一帧
  否则不得再追加第二帧
if token == 无:
  不追加新 Stage=3
