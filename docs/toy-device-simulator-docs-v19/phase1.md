...
**terminalLocked：** 调用方已持 `device_mu`；函数内不再加锁、不唤醒；写快照、释槽、经 `appendEventLocked` 写 `turn_terminal`、返回 `TerminalNotify`。解锁后再 notify completion（本阶段即 `--wait`）。Phase 1 无 REST `/wait` / WS，event waiter 与 `wsChans` 为空，仍须走 `appendEventLocked`。Phase C 只调一次，禁止再摘 Turn waiter。

**CancelTurn：** 不置 `closing`；**先**删本 Turn 未写出的 Stage 1/2，再按取消表追加 Stage 3（占 outbound 控制槽）；ACK/report 保留。已从队列取出正在 Write 的帧视为已发出。入不了队则 `stage3_backpressure`，不卡住。

**BeginClose：** 仅进程收口 / 连接失败。算法是 **过滤** 不是整队清空：丢掉未写出 Stage=1/2 与 ACK/report，**保留已入队 Stage=3**。`closing=true`。优雅关闭且 token=Stage3 时，若该 uuid 尚无 Stage=3 再追加为最后一帧。异常 / Reserved / 已 Terminal：token=无，不追加新帧，但 CancelTurn 已入队的 Stage=3 必须由 Phase B 写出后再关 socket。closer 协程跑 drain/close。

失败 JSON：读循环不得关 socket；持锁 CancelTurn + `terminalLocked(error)`，解锁后唤醒。连接保持到 CLI 退出再 BeginClose（token=无，过滤保留该 Stage=3）。
...
本阶段无 HTTP `/interrupt`。失败 JSON 仍须 CancelTurn。进程退出 BeginClose（token 按取消表；已 Terminal 则 token=无，但不得丢掉 CancelTurn 已入队的 Stage=3）。outbound `write_queue_depth` 内为 Stage=3 留 1 个控制槽。
...
ACK∥timeout 一次消费；失败 JSON 发 Stage=3 且 **连接未因 CancelTurn 关闭**；进程退出 BeginClose **过滤**未发 Stage 1/2；最后一帧 Stage=3 来自本次 token **或** 先前 CancelTurn 已入队者（已 Terminal 的 token=无 **禁止清空**它）；`terminalLocked` 不在锁内唤醒；解码失败不占槽；全部终态有 `turn_terminal` 且字段符合 §6 终止表。
