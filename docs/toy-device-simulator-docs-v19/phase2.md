# Phase 2 详细设计：多设备编排 + API + Scenario（v19）
...
`POST /devices/{id}/stop`、`DELETE /devices/{id}` 按 §4。DELETE 200 `{ "device_id","instance_id","deleted": true }`。stop/delete 走 BeginClose（**过滤**），不走 CancelTurn。若此前 `/interrupt` 或失败 JSON 已 `CancelTurn` 入队 Stage=3，BeginClose(token=无) **必须保留并写出** 该帧，禁止把 buffer 整队丢弃。
...
speak_and_wait 200 另含终态：`turn_end_reason`、`uplink_end_reason`、`reply_kind`、`event_seq`、`event_type`（通常 `turn_terminal`）。取值见 §6.7 终止表。504 时 Turn 继续。completion waiter 由 `terminalLocked` 摘出；同一临界区内 `tts_done` / `turn_terminal` 的 event waiter 由各次 `appendEventLocked` 摘出并 **累积**；外层解锁后 completion、全部 event waiter 都唤醒，并对每次事件 WS fan-out。禁止只唤醒 completion。
...
- live：先推 backlog（`seq > after`），连接保持，再推未来。未来帧来自 `appendEventLocked` 快照的 `wsChans`，在 **释放 `device_mu` 之后** fan-out；禁止在持 `device_mu` 时 `WriteMessage`。发送通道满或写出失败 → 关闭该 WS，不阻塞 Turn，不把 HTTP `/wait` 改成 504。
- tombstone：推 backlog（省略则从 oldest 到 `device_deleted`），然后 **关闭**。禁止空连接空等、禁止登记 future waiter。禁止接到新 live 的 seq。
...
| 有活动 Turn | CancelTurn + `terminalLocked(interrupt)`；**200 在 turn_terminal 写入且解锁唤醒 completion 与 event waiter、并 WS fan-out 之后** | `{ "interrupted": true, "device_id", "instance_id", "turn_id", "turn_end_reason": "interrupt", "uplink_end_reason", "reply_kind" }` |
...
- [ ] `terminalLocked` 不重复加锁；经 `appendEventLocked` 摘 event waiter 并快照 WS；同一临界区多次写事件须累积 `EventNotify`；Phase C 只迁移一次；解锁后才唤醒 completion **和** 全部 event waiter，并 WS fan-out。禁止日志已有 `turn_terminal`/`tts_done` 而 `/wait` 仍 504
- [ ] live WS 未来帧不得在 `device_mu` 内 `WriteMessage`；fan-out 失败只关该 WS
...
- [ ] 删除后 TTL 内：`GET /devices/{id}` → **404**；`GET .../events?instance_id=旧` → **200**
- [ ] wait_ready 命中 tombstone 或 committed → 409；Phase C 409 唤醒；二代 Ready 不唤醒一代
- [ ] 短锁窗口内 DELETE → speak 404；拷贝完成后 DELETE 不影响该 Turn
- [ ] BeginClose 是过滤不是清空：未发 Stage=2 不会发出；CancelTurn 已入队的 Stage=3 在随后 BeginClose(token=无) 时仍写出
- [ ] CancelTurn 先删 Stage 1/2 再入队 Stage=3。outbound 为 Stage=3 留 1 个控制槽；数据帧满不阻止 Stage=3；入不了队则 `local_validation_error`/`stage3_backpressure`，`/interrupt` 仍 200
