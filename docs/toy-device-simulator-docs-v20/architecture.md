...
| 客户端 close 帧，或 **非 deadline** 的读错误 | `abort` |
| inbox 过载 | `abort` |
| WriteMessage / Ping 超时或失败 | `abort` |
| writer 在 `idle_deadline` 看见本次 Ping 无 Pong | `abort` |

**读 deadline 到期不得 abort。** 否则 `SetReadDeadline(idle_deadline)` 会在窗口结束时杀掉已成功 Pong、本该续窗的连接。
...
| last_pong | 只在 `device_mu` 内更新。reader 收到 pong 时设为 now。 |
| writer / reader | writer 唯一 `WriteMessage` / `Ping` / `Close`（升级失败且 writer 未启动除外）。reader 只读，不得 WriteMessage。reader：**仅** close 帧与非 deadline 读错误 → `requestClose(abort)`。读 deadline 到期：不 abort，继续 Read。 |
...
| 写出 deadline | 每次 `WriteMessage`：`SetWriteDeadline(now+T)`。离开空闲开始写数据前 `SetReadDeadline(0)`（取消读超时），避免旧 `idle_deadline` 在写出期间误醒。 |
| 空闲半开（总预算一个 T） | 进入空闲时固定 `idle_deadline=now+T`，`SetReadDeadline(idle_deadline)`。`now+T/2` 仍无事件且仍 `open` → Ping，`SetWriteDeadline(idle_deadline)`（剩余时间，**禁止**再加一个 T）。Pong 只更新 `last_pong`，**不** abort。到 `idle_deadline`：**writer** 持锁看 `last_pong` 是否覆盖本次 Ping；是则新窗口并 `SetReadDeadline(新 idle_deadline)`；否则 `requestClose(abort)`。从进入该次空闲到半开 abort **≤ T**。Pong 成功后连接必须能进入下一窗口，不得因读超时被关。 |
...
writeOne(ev):
  ...
  释放
  SetReadDeadline(0)          # 离开空闲；禁止沿用旧 idle_deadline
  ...
  loop:
    持 device_mu
      ...
      若 inbox 非空:
        ...
        continue
    释放
    # 空闲：idle_start=now；idle_deadline=idle_start+T；SetReadDeadline(idle_deadline)
    等到 inbox 可读/关闭、或 now>=idle_start+T/2、或 now>=idle_deadline
      到 T/2 且尚未 Ping 且仍 open: Ping（WriteDeadline=idle_deadline）；失败 → abort()
      到 idle_deadline: 持锁看 last_pong 是否覆盖本次 Ping；
        是则新窗口、SetReadDeadline(新 idle_deadline)
        否则 requestClose(abort); abort()
      inbox 变化: continue 回 loop 顶   # 有事件则 writeOne 会清 ReadDeadline
```
...
| `GET /ws/events` | 见 §4.1：`close_mode` 区分 drain/abort；inbox 空才 live；单次写出 `now+T`；空闲 Ping+Pong 共享同一个 `idle_deadline=now+T` | 不入 hub。单协程写 backlog（含 `device_deleted`），每条 `now+T`；超时 abort。不空等 |
...
| WS | close_mode=open\|drain\|abort；inbox 只 close 一次；写出 now+T；空闲 1T 续窗看 last_pong；读 deadline 到期不 abort |
