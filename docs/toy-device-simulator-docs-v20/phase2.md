# Phase 2 详细设计：多设备编排 + API + Scenario（v20）
...
- live：持 `device_mu` 做 410 判定，拷贝 backlog 切片，空 inbox，`close_mode=open`，阶段 `catchup`，入 hub；解锁后升级，启动 writer 与 reader。writer 写完切片后 swap 到空且仍 `open` 才置 `live`。删除：`requestClose(drain)`，必须写出 `device_deleted`。reader：仅 close 帧与非 deadline 读错误 `abort`；读 deadline 到期不 abort。单次写出 `now+T`；空闲 Ping+Pong 共享 `idle_deadline=now+T`，Pong 成功必须续窗。禁止解锁 fan-out / 关 socket。
...
- [ ] live WS：inbox 空且 `open` 才置 live；删除 `drain` 写出 `device_deleted`；reader 非 deadline 错误才 `abort`。停读：卡住的那次 WriteMessage 在 T 内失败并 abort。半开空闲：从进入该次空闲起 ≤ T 退出（Ping 在 T/2，Pong 共享 idle_deadline）。Pong 成功后连接在 T 之后仍 live、进入下一窗口。读 deadline 到期不得关连接
