...
| writer / reader | writer 唯一 `WriteMessage` / `SetWriteDeadline` / `Ping`（`WriteControl`）/ `Close`（升级失败且 writer 未启动除外）。reader 只 `ReadMessage`。`ReadMessage` 错误或 close 帧 → **未持锁** `requestClose(abort)`。writer `Close` 可与 reader 并发，用于唤醒。 |
| 登记 | 持锁：410 → 拷贝切片 → 空 inbox、`catchup`、`close_mode=open` → 入 hub。禁止解锁后发 backlog 再登记。 |
| 升级失败 | 已登记则 **未持锁** `requestClose(abort)`，**HTTP 处理协程 Close**（writer 未启动，唯一非 writer Close）。 |
| 写出 deadline | 每次 `WriteMessage`：`SetWriteDeadline(now+T)`。禁止 `SetReadDeadline`。 |
| 空闲半开（总预算一个 T） | **独立 timer，不碰读 deadline。** 进入空闲时固定 `idle_deadline=now+T`，`ping_at=0`。到 `idle_start+T/2`：锁内若仍 `open` 且 `ping_at==0` 则记下 `ping_at=now`，**显式解锁后** Ping，`SetWriteDeadline(idle_deadline)`（剩余时间，禁止再加一个 T）。到 `idle_deadline`：锁内判定；仅当 `ping_at!=0` 且 `last_pong < ping_at` 才 `requestCloseLocked(abort)`，**显式解锁后** `finishAbort()`。`ping_at==0` 或 `last_pong >= ping_at` → 解锁后开新窗口。Pong 只更新 `last_pong`。从进入该次空闲到半开 abort **≤ T**。Pong 成功后必须能跨多个 T 窗口存活。禁止持锁 Ping / Close。 |
  ...
  loop:
    持 device_mu
      ...
      若 inbox 非空:
        ...
        continue
    释放
    # 空闲：idle_start=now；idle_deadline=idle_start+T；ping_at=0
    # 独立 timer；禁止 SetReadDeadline；禁止持锁 Ping/Close
    等到 inbox 可读/关闭、或 now>=idle_start+T/2、或 now>=idle_deadline
      # 多条件同时就绪：inbox 变化优先于 T/2 优先于 idle_deadline
      ...
      到 T/2 且 ping_at==0:
        持 device_mu
          ...
          ping_at = now
        释放
        SetWriteDeadline(idle_deadline)
        Ping；失败 → abort()
      ...
      到 idle_deadline:
        持 device_mu
          ...
          若 ping_at==0 或 last_pong >= ping_at:
            释放; 开新窗口（idle_start=now，idle_deadline=now+T，ping_at=0）; continue 本空闲等待
...
| WS | close_mode=open\|drain\|abort；inbox 只 close 一次；写出 now+T；空闲独立 timer 1T；禁止 SetReadDeadline；Close 唤醒 reader；requestCloseLocked 已持锁且禁止 IO；requestClose 未持锁；abort/finishAbort 未持锁；禁止二次加锁、禁止持锁 Ping/Close |
