...
- 步骤：
  ...
  3. 对 hub 中每个订阅（含阶段仍为 `catchup` 者）：若 `event_seq` 匹配过滤，**非阻塞**投入该订阅 **inbox**（不是 backlog 切片）。inbox 上限见 wsHub 两阶段。超上限 → `requestCloseLocked(abort)`，记 `slowSubs`。不得阻塞 Turn，不得在本函数 Close socket。
  4. 返回 `EventNotify`。
...
**关停入口锁所有权（Go `Mutex` 非重入；禁止「同一函数有时持锁有时未持锁」）：**

| 函数 | 调用方锁 | 做什么 | 禁止 |
|------|----------|--------|------|
| `requestCloseLocked(mode)` | **必须已持** `device_mu` | `abort` 覆盖一切；仅 `open` 可进 `drain`；摘 hub；inbox **只 close 一次**（已关闭则跳过） | 再加锁；Ping / WriteMessage / SetWriteDeadline / Close；丢弃队列 |
| `requestClose(mode)` | **必须未持** `device_mu` | 加锁 → `requestCloseLocked(mode)` → 解锁 | 调用方已持锁时再调本函数 |
| `finishAbort()` | **必须未持** `device_mu` | 丢弃未写出的切片 / drain / inbox 剩余；`Close` socket；writer 退出（**不返回**） | 再调 `requestClose` / `requestCloseLocked`；持锁调用 |
| `abort()` | **必须未持** `device_mu` | `requestClose(abort)` 然后 `finishAbort()`（**不返回**） | 持锁调用 |

close inbox 只负责唤醒，不编码模式。`device_mu` 内禁止任何 socket IO。

| 原因 | mode | 入口 |
|------|------|------|
| 删除：已 `appendEventLocked(device_deleted)` | `drain` | `requestCloseLocked`（已持 `device_mu`） |
| 客户端 close 帧，或 reader 的 `ReadMessage` 错误 | `abort` | `requestClose`（未持锁） |
| inbox 过载 | `abort` | `requestCloseLocked`（`appendEventLocked` 内） |
| WriteMessage / Ping 超时或失败 | `abort` | `abort()`（未持锁） |
| writer 在 `idle_deadline` 看见本次 Ping 无 Pong | `abort` | 锁内 `requestCloseLocked`，解锁后 `finishAbort()`（禁止再 `abort()`） |
...
| last_pong | 只在 `device_mu` 内更新。reader 在启动 `ReadMessage` 循环 **之前**（同一协程）`SetPongHandler` 一次；Pong 回调里 **加锁** 写入 now 后立刻解锁。回调禁止 IO、禁止 `requestClose*`。writer 不得调任何读侧方法。 |
| writer / reader | writer 唯一 `WriteMessage` / `SetWriteDeadline` / `WriteControl`（Ping 必须带 `idle_deadline`）/ `Close`（升级失败且 writer 未启动除外）。reader 只 `ReadMessage`。`ReadMessage` 错误或 close 帧 → **未持锁** `requestClose(abort)`。writer `Close` 可与 reader 并发，用于唤醒。 |
| 登记 | 持锁：410 → 拷贝切片 → 空 inbox、`catchup`、`close_mode=open` → 入 hub。禁止解锁后发 backlog 再登记。 |
| 升级失败 | 已登记则 **未持锁** `requestClose(abort)`，**HTTP 处理协程 Close**（writer 未启动，唯一非 writer Close）。禁止 `finishAbort`（writer 未启动；切片/inbox 随订阅丢弃）。 |
| 写出 deadline | `WriteMessage`：每次 `SetWriteDeadline(now+T)`。Ping：`WriteControl(PingMessage, nil, idle_deadline)`，**禁止**用 `SetWriteDeadline` 约束 Ping。禁止 `SetReadDeadline`。Gorilla v1.5.3 的 `WriteControl` 用参数覆盖 conn 写 deadline；零值退化为 1000h 无界等待；`deadline` 已过期立即失败。随后 `WriteMessage` 必须重新 `SetWriteDeadline(now+T)`（`writeOne` 已做）。 |
| 空闲半开（总预算一个 T） | **独立一次性 timer，不碰读 deadline。** `idle_start` / `idle_deadline` / `ping_at` 为 **writer 私有**（仅与 `last_pong` 比较时持 `device_mu` 读 `last_pong`）。进入空闲：`idle_deadline=now+T`，`ping_at=0`；arm **一次性** `t_half=idle_start+T/2` 与 **一次性** `t_end=idle_deadline`（禁止对 `now>=T/2` 忙等）。`t_half`：锁内若仍 `open` 且 `ping_at==0` 则记下 `ping_at=now`，**显式解锁后** `WriteControl(PingMessage, nil, idle_deadline)`。禁止传零值 deadline；失败或已过期 → `abort()`。Ping 后只停 `t_half`，继续等 inbox 或 `t_end`（禁止再加一个 T）。到 `t_end`：锁内判定；仅当 `ping_at!=0` 且 `last_pong < ping_at` 才 `requestCloseLocked(abort)`，**显式解锁后** `finishAbort()`。`ping_at==0` 或 `last_pong >= ping_at` → 解锁后开新窗口（重新 arm 两个一次性 timer）。从进入该次空闲到半开 abort **≤ T**。Pong 成功后必须能跨多个 T 窗口存活。禁止持锁 Ping / Close。 |
| tombstone | 不入 hub。单协程写 backlog（含 `device_deleted`），每条 `now+T`；超时则丢弃剩余并 Close（**禁止** `requestClose*`，无订阅）。客户端断开同此。不空等。 |

**writer 顺序（禁止「先置 live 再排空」；每次写出前检查 `close_mode`）：**

```text
# 下列四个函数的锁所有权见上表；伪代码里的「释放」= 先 Unlock 再调未持锁入口。

requestCloseLocked(mode):          # 已持 device_mu；禁止 IO
  abort 覆盖一切；仅 open 可进 drain
  摘 hub；inbox 只 close 一次

requestClose(mode):                # 未持锁
  Lock; requestCloseLocked(mode); Unlock

finishAbort():                     # 未持锁；writer 退出路径
  丢弃未写出的切片 / drain / inbox 剩余
  Close socket；退出               # 禁止再 requestClose*
...
loop:  # catchup
  持 device_mu
    ...
    若 inbox 空:
      若 close_mode==drain: 释放; Close; 退出   # 已解锁；drain 排空，禁止 finishAbort
      phase=live; 释放; break
...
live:
  loop:
    持 device_mu
      若 close_mode==abort: 释放; abort()
      若 close_mode==drain:
        若 inbox 空: 释放; Close; 退出   # 已解锁；drain 排空，禁止 finishAbort
        drain = 取出全部; 释放
        对 drain 每条: writeOne(ev)
        continue
      若 inbox 非空:
        ...
        continue
    释放
    # 空闲：idle_start=now；idle_deadline=idle_start+T；ping_at=0（writer 私有）
    # arm 一次性 t_half 与 t_end；禁止 SetReadDeadline；禁止持锁 Ping/Close
    等到 inbox 可读/关闭、或 t_half 到期、或 t_end 到期
      # 多条件同时就绪：inbox 变化优先于 t_half 优先于 t_end
      inbox 变化: continue 回 live loop 顶
      t_half 到期且 ping_at==0:
        持 device_mu
          若 close_mode != open: 释放; continue 回 live loop 顶
          若 inbox 非空或已关闭: 释放; continue 回 live loop 顶
          ping_at = now
        释放
        WriteControl(PingMessage, nil, idle_deadline)
          # 禁止 SetWriteDeadline 约束 Ping；禁止零值 deadline
          # 失败或 deadline 已过期 → abort()
        # 停 t_half；同一窗口继续等 inbox 或 t_end，不新开一个 T
      t_end 到期:
        持 device_mu
          若 close_mode==abort: 释放; abort()
          若 close_mode==drain 或 inbox 非空或已关闭: 释放; continue 回 live loop 顶
          若 ping_at==0 或 last_pong >= ping_at:
            释放; 开新窗口（idle_start=now，idle_deadline=now+T，ping_at=0，重新 arm t_half 与 t_end）; continue 本空闲等待
          requestCloseLocked(abort)
        释放
        finishAbort()              # 禁止再 abort() / requestClose
```
...
删除步骤：如需则 wait finalizer → 持 `device_mu`：`acc += appendEventLocked(device_deleted)`（只 push **inbox**）→ `requestCloseLocked(drain)`（**不** Close socket、**不**再加锁）→ 整份 log 移入 tombstone → 摘 live → 解锁后 `notify(acc.eventWaiters)`。writer 按 `drain` 写完剩余（含 `device_deleted`）；若期间 reader 失败则 **未持锁** `requestClose(abort)` 覆盖，放弃剩余。禁止在 push 之前 `requestCloseLocked`。`device_deleted` 的 HTTP waiter 不得丢弃 `EventNotify`。磁盘录音保留至 TTL（与 event_log 相同），过期可删。
...
| WS | close_mode=open\|drain\|abort；inbox 只 close 一次；WriteMessage 用 SetWriteDeadline(now+T)；Ping 用 WriteControl(..., idle_deadline)，禁止零值、禁止用 SetWriteDeadline 约束 Ping；空闲一次性 timer 1T；禁止 SetReadDeadline；Close 唤醒 reader；requestCloseLocked 已持锁且禁止 IO；requestClose 未持锁；abort/finishAbort 未持锁；禁止二次加锁、禁止持锁 Ping/Close |
