# Phase 2 详细设计：多设备编排 + API + Scenario（v24）
...
- live：持 `device_mu` 做 410 判定，拷贝 backlog 切片，空 inbox，`close_mode=open`，阶段 `catchup`，入 hub；解锁后升级，启动 writer 与 reader。writer 写完切片后 swap 到空且仍 `open` 才置 `live`。删除：`requestCloseLocked(drain)`，必须写出 `device_deleted`。reader：启动前设 `SetPongHandler`，只 `ReadMessage`；错误或 close 帧 **未持锁** `requestClose(abort)`。**禁止** `SetReadDeadline`。`WriteMessage` 用 `SetWriteDeadline(now+T)`。空闲一次性 timer：T/2 **锁内**记 `ping_at`、**解锁后** `WriteControl(PingMessage, nil, idle_deadline)`（禁止用 `SetWriteDeadline` 约束 Ping，禁止零值 deadline）；到 T **锁内**判定，半开失败则 `requestCloseLocked(abort)`、**解锁后** `finishAbort()`（`Close` 唤醒 reader）。禁止解锁 fan-out / 关 socket。禁止持锁 Ping / Close，禁止对同一把 `device_mu` 二次加锁。
...
- [ ] live WS：inbox 空且 `open` 才置 live；删除 `drain` 写出 `device_deleted`；reader 只 ReadMessage，禁止 SetReadDeadline。停读：卡住的那次 WriteMessage 在 T 内失败并 abort。半开空闲：一次性 timer；T/2 `WriteControl(PingMessage, nil, idle_deadline)`（Ping 自身有界，禁止零值 deadline）；不回 Pong 或 TCP 黑洞时，从进入空闲起 ≤ T writer `finishAbort` 且 socket 已关。Pong 成功后连接跨多个 T 窗口仍 live
