...
| 阶段 / 事件 | 关联键 |
|-------------|--------|
| 握手、Connected、Registering、`ack_failure`、Registered、Reporting、`report_echo` | **`correlation_id`**（连接级；此时尚无 Turn） |
...
| `report_echo` | `'1'` + topic 以 `/report/client` 结尾，`data` 为 `ReportData` 回显（不是 AckResponse），且 `data.sequence_number` 等于该连接原子计数器**刚发出的那一次** |
...
- `tts_chunk`：匹配 `uplink_uuid` 的 `'0'` Stage=1 分片
- `tts_done`：本轮至少一帧 `tts_chunk`，且 DownlinkPlayer 因 idle（默认 20s）回到 Idle
- 零下行 timeout：按矩阵允许时记 `expected_server_drop`，`turn_end_reason=timeout`；不得标 `tts_done`

### 4.4 Scenario

主断言：`tts_done` + `turn_end_reason=idle`。  
`asr_contains` 仅当设备类型 `StreamingAsrTextReply=true`，标为 optional。  
注入类 Scenario 按 §4.5 写断言（例如 `skip_report` 不得 assert drop）。
...
| fault | 前置 | 精确出站 | 期望 |
|-------|------|----------|------|
| `skip_register` | 上列夹具新鲜 ID | 握手后不发 register；合法 `'0'` 头 100 字节 + pcm + Stage=2 | `expected_server_drop`；outbound 有音频帧 |
...
### 4.6 取消转移表（interrupt / cancel_previous）
...
| WaitingReply / 下行 PlayingTTS | 发 Stage=3 | 保持已有 | `interrupt` |
| Terminal | 无操作 | 不变 | 不变 |

然后：新 UUID、Seq=0、新 Reserved。禁止同一 UUID 交叉写帧。

### 4.7 ACK：运行时 vs 测试 fixture

**运行时（模拟器代码路径，Phase 1 与 Phase 2 相同）：**

- 音频头 `NeedAck == 1` → 必须按配置 `mode` 发 ACK。
- 指令 JSON `need_ack == 1` → 必须发 ACK。
- 标志为 0 → 不发 ACK。
- **禁止**查询、缓存或依赖服务端 `deviceType.DownlinkAck` 来决定是否 ACK。

**集成测试 fixture（套件/种子数据，不是模拟器查询）：**

- 需要报文出现 ACK 标志的用例：core 中该 `device_type` 须 `DownlinkAck=true`，否则服务端不会置位 `NeedAck`/`need_ack`，设备正确选择不 ACK。此时应判 **fixture 问题**，不是模拟器漏 ACK。
- YAML `expect_downlink_need_ack` 仅注释「本示例预期会收到标志」，**不是**运行时开关。
...
| 心跳 | 周期 **report**（不要用 register 保活） |
| Register | `/register/client` 且 `data.code==0` 才算 Registered；`5001` → `ack_failure` |
| Report | `/report/client` 且 `sequence_number` 匹配才 Ready；不是 AckResponse |
| report 序号 | 每个 DeviceInstance **一个原子递增器**；初始 report、keepalive、`POST /report` 全部使用它 |
| 线上格式 Phase 1/2 | **pcm**（raw s16le mono）；配置写成 wav 作为线上格式则启动拒绝 |
| 编码顺序 | 内部 PCM 时间轴 → 按 slice_ms 切 **pcm 字节** 成帧。禁止对每一片单独封装 WAV |
| ACK | 见 §4.7；Phase 1 用 binary 且 SleepMs=0 |
...
| 阶段 | 交付 |
|------|------|
| Phase 1 | protocol 包、Reserved 状态机、fault 矩阵、pcm 分帧、`cmd/speak` `cmd/check` `cmd/fixture`、报文驱动 ACK（SleepMs=0） |
| Phase 2 | Device Manager、REST+WS；**默认 reject，可选 cancel_previous，不交付 queue**；音频+指令 ACK 契约；查询/配置 API；Scenario |
